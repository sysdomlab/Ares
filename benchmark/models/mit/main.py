...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet18',
                    ...
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
parser.add_argument('-c', '--classes', default=1000, type=int,
                    help='num_classes of model')
...
parser.add_argument('--autoscale-bsz', dest='autoscale_bsz', default=False, action='store_true',
...
class IndexedImageFolder(Dataset):
    def __init__(self, dataset_dir, transform=None):
        ...
        idx_file = os.path.join(dataset_dir, "index.txt")

        with open(idx_file, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                info = line.strip().split(',')
                file_path, label_idx = os.path.join(dataset_dir, info[0]), int(info[1])
                self.data.append((file_path, label_idx))

    def __getitem__(self, index):
        fp, label = self.data[index]
...
def main_worker(args):
    ...
    ed = time.time()
    print("=> initializing time: {:.2f}s".format(ed - st))
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer):
    ...
    gpu_time = AverageMeter('GPU', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader.dataset),
        [batch_time, data_time, gpu_time, losses, top1, top5],
