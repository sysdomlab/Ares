...
import socket
import subprocess
from policy.gavel_policies import allox, fifo, finish_time_fairness, gandiva, isolated, \
...
def get_policy(policy_name, solver=None, seed=None,
    ...
    if policy_name.startswith('allox'):
        ...
        policy = max_min_fairness.MaxMinFairnessPolicyWithPacking(solver=solver)
        ...
        policy = max_sum_throughput.ThroughputNormalizedByCostSumWithPerf(solver=solver)
    elif policy_name == 'max_sum_throughput_normalized_by_cost_perf_SLOs':
        policy = max_sum_throughput.ThroughputNormalizedByCostSumWithPerfSLOs(solver=solver)
    elif policy_name == 'max_sum_throughput_normalized_by_cost_packed_SLOs':
        policy = max_sum_throughput.ThroughputNormalizedByCostSumWithPackingSLOs(solver=solver)
