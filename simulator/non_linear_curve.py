...
def return_err(func):
    def wrapper(app, num_replicas, global_bsz):
        for i in [1, 2, 4, 8, 16, 32, 64]:
            try:
                assert int(global_bsz / i) != 0
                step_time, sync_time = func(app, num_replicas, int(global_bsz / i))
                return (step_time - sync_time) * i + sync_time, sync_time
            except Exception as e:
                pass
        Exception(f"Error: {e} with num_replicas {num_replicas}, global_bsz {global_bsz}")
    return wrapper
...
def predict_step_time(app, num_replicas, global_bsz):
    ...
    # count = num_replicas * (accum_steps + 1)  # todo 增加profile
...
if __name__ == '__main__':
    for app_name, app in APPLICATIONS.items():
        ...
        for num_replicas in range(1, 65):
            global_bsz = app.max_local_bsz * num_replicas
            ...
            tp1 = app.max_local_bsz / step_times[1]  # 1并行单卡吞吐 sample/s
