...
if __name__ == '__main__':
    ...
    all_configs = [config for config in all_configs if config["job_name"] not in profiled_job_name]
    # 过滤现在可用机器无法测量的
    all_configs = [config for config in all_configs if
                   sum(map(lambda x: x > 0, config["placement"])) <= sum(map(lambda x: x is not None, available_node))]
    print(
        f"num_configs = {len(all_configs)}, all_configs = {all_configs}"
    )
    # 构造命令
    for config in all_configs:
        ...
        allocation_ng = [(available_node[i], j)
                         for i in range(4) if available_node[i] is not None
