...
from policy.gavel_policies.policy import Policy, PolicyWithPacking
...
class FinishTimeFairnessPolicy(Policy):
    ...
    def __init__(self, solver):
        ...
        self._finish_time_fairness_perf_policy = FinishTimeFairnessPolicyWithPerf(solver)

    def get_allocation(self, unflattened_throughputs, scale_factors,
                       unflattened_priority_weights,
                       times_since_start,
                       num_steps_remaining, total_steps, prestart_overhead, cluster_spec):
        ...
        for job_id in unflattened_throughputs:
            ...
            for worker_type in unflattened_throughputs[job_id]:
                ...
                new_unflattened_throughputs[job_id][worker_type] = unflattened_throughputs[job_id]['v100']
...
class FinishTimeFairnessPolicyWithPerf(Policy):
    ...
    def get_allocation(self,
        ...
        priority_weights = np.array([1. / unflattened_priority_weights[job_id]
                                     for job_id in job_ids])

        # Create allocation variable, and isolated allocation.
        x = cp.Variable(throughputs.shape)
        isolated_throughputs = self._isolated_policy.get_throughputs(throughputs, index, scale_factors, cluster_spec)
        ...
        for i in range(len(job_ids)):
            ...
            if job_ids[i] in self._num_steps_remaining_prev_iteration:
                self._cumulative_isolated_time[job_ids[i]] += (
                        (self._num_steps_remaining_prev_iteration[job_ids[i]] - num_steps_remaining[job_ids[i]]) /
                        self._isolated_throughputs_prev_iteration[job_ids[i]]
                )

            allocation_throughput = cp.sum(cp.multiply(throughputs[i], x[i]))

            assert (isolated_throughputs[i] > 0)
            # expected_time_isolated = self._cumulative_isolated_time[job_ids[i]] + \
            #     (num_steps_remaining[job_ids[i]] / isolated_throughputs[i])
            expected_time_isolated = prestart_overhead[job_ids[i]] + total_steps[job_ids[i]] / isolated_throughputs[i]

            expected_time_allocation = (
                    times_since_start[job_ids[i]] + num_steps_remaining[job_ids[i]] * cp.inv_pos(allocation_throughput)
            )
        ...
        for i in range(len(expected_time_fractions)):
            allocation_throughput = np.sum(np.multiply(throughputs[i], x.value[i]))
            expected_time_allocation = (times_since_start[job_ids[i]] + (
                    num_steps_remaining[job_ids[i]] * cp.inv_pos(allocation_throughput))).value
            expected_time_isolated = prestart_overhead[job_ids[i]] + total_steps[job_ids[i]] / isolated_throughputs[i]

            print(f'{job_ids[i]}')
            print(f'vars: {x.value[i]}')
            print(f'\tallocated througput: {allocation_throughput}')
            print(
                f'\texpected time allocation: {times_since_start[job_ids[i]]} + {num_steps_remaining[job_ids[i]]} * {cp.inv_pos(allocation_throughput).value} = {expected_time_allocation}')
            print(f'\tisolated time: {expected_time_isolated}')
            print(
                f'\tftf: {expected_time_allocation} / {expected_time_isolated} = {expected_time_fractions[i].value} ({expected_time_allocation / expected_time_isolated})')
        ...
        self._isolated_throughputs_prev_iteration = {}
        for i in range(m):
            self._isolated_throughputs_prev_iteration[job_ids[i]] = isolated_throughputs[i]

        if x.value is None:
            return self._isolated_policy.get_allocation(unflattened_throughputs, scale_factors, cluster_spec)
